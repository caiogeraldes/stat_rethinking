...
f <- rethinking::rbern(N_dyads, 0.1)
...
for (i in 1:N) for (j in 1:N) {
  if (i != j) {
    ...
    p_tie <- f[the_dyad] + (1-f[the_dyad])*rethinking::inv_logit(alpha)
    y[i,j] <- rethinking::rbern(1, p_tie)
  }
}
...
vcol <- "#DE536B"
plot(sng , layout=lx , vertex.size=8 , edge.arrow.size=0.75 , edge.width=2 , edge.curved=0.35 , vertex.color=vcol , edge.color=rethinking::grau() , asp=0.9 , margin = -0.05 , vertex.label=NA )
...
mGD <- ulam( f_dyad , data=sim_data , chains=4 , cores=4 , iter=2000 , file = "./models/mGD")

precis( mGD , depth=3 , pars=c("a","Rho_T","sigma_T") )
plot(precis( mGD , depth=3 , pars=c("a","Rho_T","sigma_T")))
